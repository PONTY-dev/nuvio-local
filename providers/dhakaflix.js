...
//  Handles year‑range folders like (1960-1994) correctly
//  Based on CloudStream Kotlin plugin structure
// ============================================================
...
function searchMovieOnServer(server, title, year) {
  ...
  // Process each movie category sequentially
  function processCategory(categoryPath) {
    var categoryUrl = serverUrl + categoryPath;
    if (categoryUrl.slice(-1) !== "/") categoryUrl += "/";

    // Step 1: Get list of year folders (e.g., "(1960-1994)", "(1995)", etc.)
    return fetchDir(categoryUrl).then(function(yearFolders) {
      // Only folders (href ends with /)
      var folders = yearFolders.filter(function(f) { return f.href.endsWith("/"); });
      // Process each year folder sequentially
      function processYearFolder(index) {
        if (index >= folders.length) return null;
        var yearFolder = folders[index];
        var yearFolderUrl = categoryUrl + yearFolder.href;
        // Step 2: Inside year folder, get movie folders
        return fetchDir(yearFolderUrl).then(function(movieFolders) {
          var movieCandidates = movieFolders.filter(function(f) { return f.href.endsWith("/"); });
          // Find the best matching movie folder
          var best = null, bestScore = 0;
          for (var i = 0; i < movieCandidates.length; i++) {
            var m = movieCandidates[i];
            var normFolder = normalize(m.name);
            var normTitle = normalize(title);
            var containsTitle = normFolder.indexOf(normTitle) !== -1;
            var containsYear = year && m.name.indexOf("(" + year + ")") !== -1;
          ...
          if (!best || bestScore < 0.8) {
            ...
            return processYearFolder(index + 1);
          }
          // Step 3: Inside movie folder, find a video file
          var movieUrl = yearFolderUrl + best.href;
...
function searchTvEpisodeOnServer(server, title, year, season, episode) {
  ...
  function processCategory(categoryPath) {
    var categoryUrl = serverUrl + categoryPath;
    if (categoryUrl.slice(-1) !== "/") categoryUrl += "/";
    return findSeriesFolderRecursive(categoryUrl, title, year).then(function(seriesFolder) {
  ...
  for (var c = 0; c < server.tvCategories.length; c++) {
    chain = chain.then(function(prevResult) {
      if (prevResult) return prevResult;
      return processCategory(server.tvCategories[c]);
    });
  }
  return chain;
}
