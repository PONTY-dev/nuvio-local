  ...
  return fetch('https://api.themoviedb.org/3/' + (isMov ? 'movie' : 'tv') + '/' + tmdbId + '?api_key=' + TMDB_KEY)
    ...
    .then(function(data) {
      ...
      if (isMov) {
        ...
        // Folder title variants (handle colon titles like "Avatar: Fire and Ash")
        var folderTitles = [title];
        if (ht !== title) folderTitles.push(ht);

        // [folderSuffix, filename, quality, source]
        var combos7 = [
          // NF space format (confirmed: A Journey, 27 Nights, Back in Action)
          ...
          // AMZN space format (confirmed: A Game in the Woods, Love Me Love Me)
          ...
          [' 720p AMZN [Dual Audio]', dt+'.'+year+'.720p.AMZN.WEBRip.x264-GalaxyRG.mkv',                                 '720p','AMZN'  ],
          // AMZ variant
        ...
        var out = [], seen = {}, i, j, ft2, folder, file, url, q, src;

        for (i = 0; i < folderTitles.length; i++) {
          ft2 = folderTitles[i] + ' (' + year + ')';

          for (j = 0; j < combos7.length; j++) {
            folder = ft2 + combos7[j][0];
            file   = combos7[j][1];
