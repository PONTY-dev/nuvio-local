...
function best(entries, target) {
  var b = null, t = 0;
  entries.forEach(function(e){ var s=score(e,target); if(s>t){t=s;b=e;} });
  return t >= 0.5 ? b : null;
}
...
function getMovieStreams(title, year) {
  ...
  return get(yUrl)
    .then(function(html) {
      var b = best(parseDir(html), title);
      if (!b) throw new Error('no folder for: ' + title);
      return get(yUrl + b).then(function(html2) {
        var files = parseDir(html2).filter(function(f){ return /\.(mkv|mp4|avi)$/i.test(f); });
        if (!files.length) throw new Error('no files in: ' + decode(b));
        return files.map(function(f) {
          var q = /1080p/i.test(f)?'1080p':/720p/i.test(f)?'720p':'SD';
          return { name:'DhakaFlix '+q, title:title+' ('+year+')', url:yUrl+b+f, quality:'BDIX', headers:{'User-Agent':UA} };
        });
      });
    })
    .catch(function(e){ console.error('[DhakaFlix Movie] '+e.message); return []; });
}
...
function getTvStreams(title, season, episode) {
  ...
  return get(rUrl)
    .then(function(html) {
      var b = best(parseDir(html), title);
      if (!b) throw new Error('show not found: '+title);
      return get(rUrl+b).then(function(html2) {
        var entries = parseDir(html2);
        ...
        return get(rUrl+b+sf).then(function(html3) {
          var files = parseDir(html3).filter(function(f){ return /\.(mkv|mp4|avi|m3u8)$/i.test(f); });
          var epf   = files.find(function(f){ return epRe.test(decode(f)); });
          if (!epf) throw new Error(ep+' not found');
          return [{ name:'DhakaFlix TV', title:title+' '+ep, url:rUrl+b+sf+epf, quality:'BDIX', headers:{'User-Agent':UA} }];
        });
      });
    })
    .catch(function(e){ console.error('[DhakaFlix TV] '+e.message); return []; });
}
...
function getStreams(tmdbId, media) {
  ...
  return fetch('https://api.themoviedb.org/3/'+(isMov?'movie':'tv')+'/'+tmdbId+'?api_key='+TMDB_KEY)
    ...
    .catch(function(e){ console.error('[DhakaFlix] '+e.message); return []; });
}
